...
<div>
    ...
    <x-input type="text" name="name" value="{{ old('name', $activeIngredient->name ?? '') }}"
        placeholder="Ej. DIMETOMORFO" required />
...
<div class="mt-6">
    ...
    <textarea name="description" rows="3" class="w-full rounded-xl border border-gray-300 px-4 py-3"
        placeholder="Descripción opcional del ingrediente activo">{{ old('description', $activeIngredient->description ?? '') }}</textarea>
...
<div class="mt-6">
    ...
    <select name="is_active" class="w-full rounded-xl border border-gray-300 px-4 py-3">
        ...
        <option value="1" {{ old('is_active', $activeIngredient->is_active ?? 1) == 1 ? 'selected' : '' }}>
            Activo
        </option>

        <option value="0" {{ old('is_active', $activeIngredient->is_active ?? 1) == 0 ? 'selected' : '' }}>
            Inactivo
        </option>
...
<div class="mt-8">
    ...
    <div class="mb-4">
        ...
        <p class="mt-1 text-sm text-gray-500">
            Selecciona los productos que contienen o corresponden a este ingrediente activo.
        </p>
    ...
    @php
        ...
        $selectedProducts = old(
            'products',
            isset($activeIngredient) ? $activeIngredient->products->pluck('id')->toArray() : [],
        );
